...
save.MCMC <- function (x) {
    
    tm <- gsub(" ", "_", gsub(":", "",
                              strsplit(gsub("-", "_",
                                            paste(Sys.time())), "\\.")[[1]][1]))

    if (exists("model.name", x)) {

        fname <- paste(x$model.name, tm, ".RData", sep="")
        
    } else {
        cat(tm)

        fname <- paste("chains_", tm, ".RData", sep="")
    }
...
rmvnorm.cond.precision.old <- function (m, P, eigen.P) {
## ======================================================================
## Purpose: Sample from N( P^{-1} m, P^{-1} )
## Assumes: m and P are real-valued.
##          The length of m is equal to the dimension of the square matrix P.
